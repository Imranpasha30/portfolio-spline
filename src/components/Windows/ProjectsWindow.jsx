...
import { ExternalLink, Github, Sparkles, Star, ArrowUpRight, Rocket } from 'lucide-react';
...
const ProjectsWindow = ({ id, isMinimized, onClose, onMinimize, zIndex = 40, offsetX = 200, offsetY = 100, onFocus }) => {
    ...
    return (
        <WindowShell
            ...
            onFocus={onFocus}
        >
            <div className="flex-1 overflow-hidden bg-gradient-to-br from-gray-900 via-gray-900 to-gray-800 flex flex-col h-full">
                {/* Header Section */}
...
const ProjectCard = ({ project, index }) => {
    ...
    const imageHeight = { small: 'h-40', medium: 'h-52', large: 'h-64' }[project.size];
            ...
            <div className="relative bg-gray-800/50 backdrop-blur-xl rounded-2xl overflow-hidden border border-gray-700/50 shadow-2xl hover:shadow-cyan-500/20 transition-shadow duration-500">
                ...
                <div className="p-6" style={{ transform: "translateZ(50px)" }}>
                    <motion.h3
                        ...
                        animate={{ color: isHovering ? project.accentColor : "#ffffff" }}
                    >
                        {project.title}
                        {isHovering && (
                    ...
                    <div className="flex flex-wrap gap-2">
                        {project.tags.map((tag, i) => (
                            <motion.span
                                ...
                                initial={{ opacity: 0, scale: 0 }}
                                animate={{ opacity: 1, scale: 1 }}
                                transition={{ delay: index * 0.1 + i * 0.05, type: "spring", stiffness: 400, damping: 15 }}
